(function () {
  ...
  b3e.draw.randomselectorsSymbol = function (block, settings) {
    var shape = new createjs.Shape();
    ...
    shape.graphics.setStrokeStyle(swidth, "round");
    shape.graphics.beginStroke(scolor);
    shape.graphics.arc(-ssize + 10, -ssize, ssize, 3.141561, 1.570796, false);
    shape.graphics.lineTo(-ssize + 10, ssize);
    shape.graphics.beginFill(scolor);
    shape.graphics.drawCircle(-ssize + 10, ssize * 2, swidth / 2);
    shape.graphics.endFill();
    shape.graphics.endStroke();
  ...
  b3e.draw.SYMBOLS = {
    ...
    RandomSelectors: b3e.draw.randomselectorsSymbol,
  };
})();
